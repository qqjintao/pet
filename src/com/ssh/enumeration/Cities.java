...
public class Cities {
	...
	public ArrayList getCities(Provinces provinces){
		switch(provinces){
		case 北京市:
			for(Beijing beijing:Beijing.values()){
				City.add(beijing);
			};break;
		case 上海市:
			for(Shanghai shanghai:Shanghai.values()){
				City.add(shanghai);
			};break;
		case 广东省:
			for(Guangdong guangdong:Guangdong.values()){
				City.add(guangdong);
			};break;
		case 山东省:
			for(Shangdong shandong:Shangdong.values()){
				City.add(shandong);
			};break;
		case 江苏省:
			for(Jiangsu jiangsu:Jiangsu.values()){
				City.add(jiangsu);
			};break;
		case 河南省:
			for(Henan henan:Henan.values()){
				City.add(henan);
			};break;
		case 河北省:
			for(Hebei hebei:Hebei.values()){
				City.add(hebei);
			};break;
		case 浙江省:
			for(Zhejiang zhejiang:Zhejiang.values()){
				City.add(zhejiang);
			};break;
		case 香港特别行政区:
			for(Hongkong xianggan:Hongkong.values() ){
				City.add(xianggan);
			};break;
		case  陕西省:
			for(Xian xian:Xian.values()){
				City.add(xian);
			};break;
		case  湖南省:
			for(Hunan hunan:Hunan.values()){
				City.add(hunan);
			};break;
		case  重庆市:
			for(Chongqing chongqing:Chongqing.values()){
				City.add(chongqing);
			};break;
		case  福建省:
			for(Fujiang fujiang:Fujiang.values()){
				City.add(fujiang);
			};break;
		case  天津市:
			for(Tianjing tianjing:Tianjing.values()){
				City.add(tianjing);
			};break;
		case  云南省:
			for(Yunnan yunnan:Yunnan.values()){
				City.add(yunnan);
			};break;
		case  四川省:
			for(Sichuan sichuan:Sichuan.values()){
				City.add(sichuan);
			};break;
		case  广西壮族自治区:
			for(Guangxi guangxi:Guangxi.values()){
				City.add(guangxi);
			};break;
		case  安徽省:
			for(Anhui anhui:Anhui.values()){
				City.add(anhui);
			};break;
		case  海南省:
			for(Hainan hainan:Hainan.values()){
				City.add(hainan);
			};break;
		case  江西省:
			for(Jiangxi jiangxi:Jiangxi.values()){
				City.add(jiangxi);
			};break;
		case  湖北省:
			for(Hubei hubei:Hubei.values()){
				City.add(hubei);
			};break;
		case  山西省:
			for(Shanxi shanxi:Shanxi.values()){
				City.add(shanxi);
			};break;
		case  辽宁省:
			for(Liaoning liaoning:Liaoning.values()){
				City.add(liaoning);
			};break;
		case  台湾省:
			for(Taiwan taiwan:Taiwan.values()){
				City.add(taiwan);
			};break;
		case  黑龙江:
			for(Heilongjiang heilongjiang:Heilongjiang.values()){
				City.add(heilongjiang);
			};break;
		case  内蒙古自治区:
			for(Neimenggu neimenggu:Neimenggu.values()){
				City.add(neimenggu);
			};break;
		case  澳门特别行政区:
			for(Aomen aomen:Aomen.values()){
				City.add(aomen);
			};break;
		case  贵州省:
			for(Guizhou guizhou:Guizhou.values()){
				City.add(guizhou);
			};break;
		case  甘肃省:
			for(Gansu gansu:Gansu.values()){
				City.add(gansu);
			};break;
		case  青海省:
			for(Qinghai qinghai:Qinghai.values()){
				City.add(qinghai);
			};break;
		case  西藏区:
			for(Xizang xizang:Xizang.values()){
				City.add(xizang);
			};break;
		case  吉林省:
			for(Jilin jilin:Jilin.values()){
				City.add(jilin);
			};break;
		case  宁夏回族自治区:
			for(Ningxia ningxia:Ningxia.values()){
				City.add(ningxia);
			};break;
		default:
				City.add("");break;	
	...
	public Provinces getProvinces(String province){
		for(Provinces p:Provinces.values()){
			if(province.equals(p.toString())){
				return p;
			}else{
